...
def prepare_data(data, so_elapsed_time, filters):
	...
	for row in data:
		...
		row["total_payment"] = get_total_payment(row.sales_order)

		row["variance_amount"] = row["total_payment"] - row["delivered_qty_amount"]
...
def get_total_payment(sales_order):
    ...
    total_payment = sum([payment['paid_amount'] for payment in payments])
...
def get_columns(filters):
  ...
  {"label": _("Order Note"), "fieldname": "order_note", "fieldtype": "Data", "width": 130},
		{
			"label": _("Sales Order"),
